...
agree to terms, and receive confirmation that their account is ready to use.
...
a strong password; verify that the account is persisted, the password is not
stored in plaintext, and the user receives a clear success confirmation. Submit
again with the same email and verify rejection.
...
1. **Given** no account exists for `alice@example.com`, **When** Alice submits
   ...
   **Then** an account is created and Alice sees a success confirmation.
2. **Given** an account already exists for `alice@example.com`, **When** Alice
   tries to register again with the same email, **Then** registration is
   rejected with a clear, non-enumerating message and no duplicate account is
   created.
3. **Given** Alice submits a password that fails the strength rules, **When**
   she submits the form, **Then** registration is rejected and the rules are
   shown.
4. **Given** Alice submits a syntactically invalid email, **When** she submits
...
1. **Given** Alice has a registered account, **When** she submits the correct
   email and password, **Then** the system issues an authentication token and
   her session is considered active.
2. **Given** Alice submits a wrong password, **When** she attempts to log in,
   **Then** the system rejects the attempt with a generic error that does not
   disclose whether the email is registered.
3. **Given** Alice submits a non-existent email, **When** she attempts to log
   in, **Then** the system returns the same generic error as the wrong-password
   case (no account enumeration).
4. **Given** Alice has failed login N consecutive times within a short window,
...
- **FR-007**: System MUST allow a registered user to authenticate using their
  email and password.
...
- **FR-010**: System MUST throttle repeated failed login attempts per account
  and per source IP, applying progressive delays or temporary lockout after a
...
- **FR-023**: System MUST log security-relevant events (registration, login
  ...
  session revocation) with timestamp, account identifier, and source IP, and
  MUST NOT log passwords or full tokens.
- **FR-024**: System MUST transport all credentials, tokens, and reset links
  only over encrypted channels (TLS).

### Key Entities *(include if feature involves data)*

- **User Account**: represents a person who can authenticate. Key attributes:
  unique identifier, email (unique, case-insensitive), password hash, account
  status (active / locked / disabled), created-at, last-login-at.
- **Session**: represents an authenticated session bound to a user. Key
  attributes: session identifier, owning user, issued-at, expires-at (= issued
  + 24h), revoked flag, source-IP/user-agent metadata for audit.
...
- **Security Event**: an audit record of a security-relevant action. Key
  ...
  outcome (success/failure), reason code.
...
- **SC-001**: A new user can complete registration end-to-end (form submit →
  account ready) in under 60 seconds on a typical broadband connection.
...
- **SC-007**: After 5 consecutive failed login attempts on an account within 5
  minutes, the 6th attempt from the same source is rejected or delayed without
...
- The application has access to an outbound transactional email service capable
  of delivering reset emails reliably; configuring that service is a
...
- The user's explicit choice of "JWT" for login tokens is honoured as the
  ...
  managed outside this feature's scope.
...
- A "session" maps 1:1 to an issued token; there is no server-side sliding
  renewal in this version (token is fixed-lifetime).
